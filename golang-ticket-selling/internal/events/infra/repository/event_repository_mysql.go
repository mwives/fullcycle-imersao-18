...
func (r *mysqlEventRepository) FindEventByID(eventID string) (*domain.Event, error) {
	...
	for rows.Next() {
		var (
			eventIDStr, eventName, eventLocation, eventOrganization, eventRating, eventImageURL sql.NullString
...
func (r *mysqlEventRepository) FindSpotByName(eventID, spotName string) (*domain.Spot, error) {
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, domain.ErrSpotNotFound // Spot not found
		}
		return nil, err
	}
...
func (r *mysqlEventRepository) CreateEvent(event *domain.Event) error {
	...
	_, err := r.db.Exec(
		query,
		event.ID, event.Name, event.Location, event.Organization, event.Rating, event.Date.Format("2006-01-02 15:04:05"), event.ImageURL, event.Capacity, event.Price, event.PartnerID)
	return err
}
...
func (r *mysqlEventRepository) ReserveSpot(postID, ticketID string) error {
	query := `
		UPDATE spots
		SET
			status = ?,
	...
	_, err := r.db.Exec(query, domain.SpotStatusSold, ticketID, postID)
	return err
}
